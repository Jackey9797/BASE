...
class base_framework: 
    ...
    def static_strategy(self):    
        ...
        # print parameters of every layer of the model
    ...
    def train(self): 
        ...
        for epoch in range(self.args.epoch): #* train body 
            ...
            for batch_idx, (data, batch_x_mark, batch_y_mark) in enumerate(self.train_loader):
                def checkStationary(batch_x, batch_y): 
                    ...
                    if np.std(batch_x) > 1.1 : 
                        return 0   
                    return 1
                # if checkStationary(data.x.reshape(-1, self.args.graph_size, self.args.x_len).T, data.y.reshape(-1,self.args.graph_size, self.args.y_len+self.args.label_len).T) == 0 : 
                #     continue 
            ...
            # print('Updating learning rate to {}'.format(lr_scheduler.get_last_lr()[0]))
    ...
    def run(self): 
        ...
        for phase in range(self.args.begin_phase, self.args.end_phase+1): ##todo 将第一年拿出来单独处理 
